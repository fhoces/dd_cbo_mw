...
param.factor.1 <- 1
param.net.benef <- 2e9
param.ripple <- c("scope_below" = 8.7, "scope_above" = 11.5, "intensity" = 0.5)
param.eta.lit <- 1
...
add.base.vars <- function(SA.hours = param.hours, 
                          ...
                          SA.N = param.N) {
  df %>% mutate("hrslyr" = hrslyr * param.hours, 
                "wkslyr" = wkslyr * param.weeks, 
                "hhwgt" = hhwgt * param.N)
}
...
add.nocomp <- function() {
  df %>% mutate("no.comply" = ifelse(pop_of_int==1,   
                                    ifelse(runif( length(new.wage) )< alpha.1, 
                                           "Not comply", "comply"),
                                    NA), 
...
win.loss.f <- function(SA.factor.1 = param.factor.1, 
  ...
  bins <- with(df,wtd.quantile(x = sq.inc.pc, probs = 1:9/10, weights = hhwgt.2016))
...
dist.pref.f <- function(x, param.dist.pref) 1/(x^param.dist.pref)
# sum cost a and benefits at the individual level
final_dec.f <- function(x) {
  sum( (df$winners - df$losers - df$balance.loss) * df$hhwgt.2016 * 
         dist.pref.f(df$income.group, param.dist.pref = x) ) /1e9
}
...
plot(seq(-.5,6,by = 0.1),sapply(seq(-.5,6,by = 0.1) ,final_dec.f), type = "l", xlab = "rho", ylab = "FE (billions of $)  ", main = "Final effect of minimum wage raises \nfor different redistributory preferences")
abline(h=0, v=0, lty = 3)
segments(x0 = c(-0.3, 0, 0.3, 0), x1 = c(-0.3, -0.3, 0.3, 0.3 ) , 
         y0 = c(0,final_dec.f(-0.3), 0,final_dec.f(0.3) ), 
         y1 = c(final_dec.f(-0.3), final_dec.f(-0.3), final_dec.f(0.3), final_dec.f(0.3)  ), col = "red", lty = 1)
```  
...
```{r sa, final results, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
# 121 * 1.05 * 0.144  * .37 * (1 - .13) * 1/3 * 0.1 + 
#   4 * 1.05 * 0.74   * .37 * (1 - .18) * 0.1
rm(list = setdiff(ls(), lsf.str()))

v0 <-  c(-9.807995,  5.669210)
...
abs(sapply(c(-0.3, 0.3), final_dec.f) - v0)/v0
