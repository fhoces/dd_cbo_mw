...
output.template2["Balance losses", ] <- c("~0.3", "~3.4", "~3.4", "~17")
output.template2["Net effect", ] <- c("5", "12", "2", "-17")
...
```{r non compliesrs in asec,  eval=TRUE,echo=TRUE, warning=FALSE, message=FALSE}
#AQUI VOY
# Apply ripple effects
...
```{r job destruction,  eval=TRUE,echo=TRUE, warning=FALSE, message=FALSE}  
...
```{r income,  eval=TRUE,echo=TRUE, warning=FALSE, message=FALSE, results='hide'}
...
```{r final results, eval=TRUE,echo=TRUE, warning=FALSE, message=FALSE}
...
list.of.packages <- c("gtrendsR", "dplyr", "ggplot2", "grid")
...
gconnect("any25996@gmail.com", "paratodos")
#gconnect("fhoces", "Benja2014")


res <- gtrends(c("minimum wage"))

res$trend %>%
  ggplot(aes(start, minimum.wage.)) + 
  geom_line(lwd = 1) +
  #stat_smooth(span = .02) +
  coord_cartesian(ylim = c(0,100), 
                  xlim = c(as.POSIXct( c("2006-01-01 12:00:00","2016-02-01 12:00:00") ) ) ) +
  geom_vline(xintercept = as.numeric(as.POSIXct( c("2014-01-21 14:00:00",
                                                   "2007-07-01 12:00:00") )), 
             col="red", size =1,  linetype=4, alpha =.5) +
  theme_minimal() + 
  labs(y = "Search Intensity", 
     x = NULL , 
     title = "Google Search for 'Minimum Wage' in the US") +
  annotate("text", label = c("last MW raised", "CBO Report on MW")
          , x = (as.POSIXct(c("2007-07-01 12:00:00", "2014-02-01 12:00:00"))), 
          y = 15, size = 4, colour = "red")+
  ggsave("min_wage_gtrend.pdf", width = 20, height = 5)
