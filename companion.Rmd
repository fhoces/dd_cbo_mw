...
MW' + 0.5(11.5 - w) \quad if \quad  w \in [10.10, 11.5) \\
MW' \quad o/w
\end{cases}
$$
...
df <- df %>% mutate("no.comply" = ifelse( pop_of_int==1,   
                    ifelse(runif( length(new.wage) )< alpha.1, 
                           "Not comply", "comply"),
                    NA), 
                    "new.wage" = ifelse(no.comply == "Not comply", 
                                        wage.adj.min, 
                                        new.wage) 
) 
...
N_benes_compliance_below_min <-  sum(df$hhwgt.2016[(df$wage.adj.min != df$new.wage) & df$below_min==1], na.rm = TRUE)/1e6
N_benes_compliance_above_min <-  sum(df$hhwgt.2016[(df$wage.adj.min != df$new.wage) & df$below_min==0], na.rm = TRUE)/1e6
N_benes_compliance<- sum(df$hhwgt.2016[(df$wage.adj.min != df$new.wage)], na.rm = TRUE)/1e6
...
wage.loss.total <- with(df, 
                        sum( (wage.adj.min - cut.wage) * 
                              hhwgt.2016 * (hrslyr * wkslyr) , na.rm = TRUE ) 
                        )/1e9
```  
