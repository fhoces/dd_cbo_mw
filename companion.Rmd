...
#### Loading the data  
```{r loading data cps_asec, eval=TRUE,echo=TRUE, warning=FALSE, message=FALSE, cache=FALSE}
# Loading required libraries
list.of.packages <- c("foreign", "dplyr", "weights", "survey", "Hmisc", "openxlsx", "rio")
new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
if(length(new.packages)) install.packages(new.packages)
...
#### Computing wages  
```{r computing wages from CPS ASEC,  eval=FALSE, echo=TRUE, warning=FALSE, message=FALSE}  
# Workers’ hourly wages were calculated as their annual earnings divided by the number of hours they 
# worked during the year (calculated as the number of hours they usually worked per week times the number of weeks they worked during the year)
asd  <- function(new.wage)  (new.wage - 7.25)*.5*( 1 + 5*(10.10 - new.wage)/new.wage )  + new.wage
hwr_wage = with(df, incp_ern/(uhours * weeks) )  
hwr_wage[hwr_wage<0] = NA

```  


#### Code for descriptive stats
```{r Descript stats1 cps asec, eval=TRUE,echo=TRUE, warning=FALSE, message=FALSE, cache=FALSE}
library(dplyr)
library(foreign)
library(weights)
library(survey)
...
table_6 <- df %>% 
  filter(pop_of_int == 1 & !is.na(hhwgt)) %>%
    select(incp_ern, hrslyr, wkslyr, hhwgt) 

table_6 <- sapply(table_6[,c("incp_ern", "hrslyr", "wkslyr")],function(x) sum.stas1(x, table_6$hhwgt) )
table_6 <- cbind(table_6)
colnames(table_6) <- c("Earnings", "Hours", "weeks") 

df$wage <- with(df, incp_ern/(hrslyr * wkslyr) )
...
$$
\begin{align}
\widetilde{w_{ig}} &= (1  + I(U \geq 0) \times F_{1}) w_{ig}I(g \in G_{1}) + w_{ig}( 1- I(g \in G_{1}))  \quad \text{with:  } U\sim Uniform(a,b) 
\end{align}
$$ 
...
df <- df %>%
        mutate( w3.deciles = cut(wage, c(0, aux.var, Inf) , 
                                right = TRUE, include.lowest = TRUE) ,  
                w3.adj1 =  wage * ( 1 + wage.gr.bins[w3.deciles] )^3) 
...
table_4[1,1] <- table_1[3]
table_4[1,2] <- new_total_n
table_4[2,1] <- round( with(df[pop_of_int == 1 & !is.na(df$wage), ], 
                            wtd.quantile( wage, weights = hhwgt
                                          , prob = c(.5) ) ), digits = 2 )

table_4[2,2] <- round( with(df[pop_of_int == 1 & !is.na(df$w3.adj.min), ], 
                            wtd.quantile( w3.adj.min, weights = hhwgt * 
                            (1 + workers.gr)^3, prob = c(.5) ) ), digits = 2 )
...
aux.1 <- df %>% 
  filter(pop_of_int == 1 & !is.na(w3.adj.min)) %>% 
    summarise("> $7.50" = weighted.mean(w3.adj.min<7.5,w = hhwgt * (1 + workers.gr)^3), 
              "> $9" = weighted.mean(w3.adj.min<9,w = hhwgt * (1 + workers.gr)^3), 
              "> $10.10" = weighted.mean(w3.adj.min<10.10,w = hhwgt * (1 + workers.gr)^3), 
              "> $13" = weighted.mean(w3.adj.min<13,w = hhwgt * (1 + workers.gr)^3), 
              "> $15" = weighted.mean(w3.adj.min<15,w = hhwgt * (1 + workers.gr)^3) 
              )
...
```{r, eval=FALSE}
imp.wage  <- ifelse(df$w3.adj.min<10.10 & pop_of_int==1, 10.10, df$w3.adj.min)
gross.gain <- sum((imp.wage - df$w3.adj.min)*df$hhwgt*df$hrslyr*df$wkslyr, na.rm = TRUE)/1e9
num.benes <- sum((imp.wage != df$w3.adj.min)*df$hhwgt, na.rm = TRUE)/1e6
```` 


## Destroying jobs  
The imputation above so far is applied to all workers below the minimum wage (and the ripple effects). Now we need to remove the $\widehat{\Delta E}$ workers by imputing them a wage of 0. CBO chose to not move the wage all the way to 0 but to cut it in half and apply such imputation to $2\widehat{\Delta E}$. When destroying jobs CBO argued that the effect would be heavier on teenagers and low wage adutls. We implement this in the follwogin algorithm:
