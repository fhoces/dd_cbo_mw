...
list.of.packages <- c("knitr","foreign", "dplyr", "weights", "survey", "Hmisc", 
                      "openxlsx", "rio", "highr", "XML", "RCurl", "treemap", "reshape2")
...
# For this chunk we need to have Java installed and up to date
...
```{r Target population, eval=TRUE,echo=TRUE, warning=FALSE, message=FALSE, results="hide", error=FALSE, collapse=TRUE, fig.show='hold'}
library(treemap)
...
Looking at historical trends in the CPS, CBO estimates that $F_{adjs}$ is 4.5 for both populations.  In the following table we summarize all the elemnts required to compute $\widetilde{ \eta^{g}_{w\leq MW} }$ 
...
stats3 <- cbind(stats3, aux1[,2])

t(stats3)

# As it should have been computed according to doc
delta.e1 <- sum( stats3$N * stats3$`% Empl Below MW`/100 * ( 1 - stats3$`% of non compliers with adj`/100 - 0) *
       stats3$`% Mean Wage Inc`/100 * 0.1/(stats3$`% Empl Below MW`/100) *
       ((stats3$`% Mean Wage Inc`/100)/0.37) * c(1/3, 1) 
) - 0.05 * 0.9
# As described in doc
delta.e2 <- sum( stats3$N * stats3$`% Empl Below MW`/100 * ( 1 - stats3$`% of non compliers with adj`/100 - 0) *
       stats3$`% Mean Wage Inc`/100 * 0.1 * 4.5 * c(1/3, 1) ) - 0.05 * 0.9 
...
$$  
 ...
 all the components descbribed above we get $\widehat{ \Delta^{-} E } =$ `r round(delta.e1*1000)` thousand jobs. The report however computes $F_{adjs}$ in a different fashion and gets a value of 4.5[^2]. Plugging this value into the equation above one gets  thousand jobs. [FH: let's not get super excited yet. I suspect that several components of my estimation are different from the ones in the CBO report (e.g. non-compliance rate)]
