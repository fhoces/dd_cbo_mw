...
billion dollars (in 2016) and a total wage loss of `r pt(wage.gain.total["Total wage loss"])` due to workers loosing their jobs. The funds that cover the wage gains, -XXXXXXX (or XXXXXXX - alpha*YYYYYY), have to come from either less profits for business or higher prices for consumers. In the next section we review the distribution of wage gains, wage losses and balance losses across the income distribution. 
...
```{r income,  eval=TRUE,echo=TRUE, warning=FALSE, message=FALSE}
...
df <- df %>% 
  ...
  mutate("N.fam"= n(), 
         ...
         "fam.wgt" = sum(hhwgt.2016, na.rm = TRUE) )


# Computing differences in income 
df %>% select(fam.new.income,fam.sq.income, perno,hhwgt.2016, N.fam) %>% 
  filter(perno == 1) %>% 
  with(sum( (fam.new.income - fam.sq.income) * hhwgt.2016, na.rm = TRUE) ) /1e9

#AQUI VOY - UNDERSTAN INCOME WELL!!!!!

# Compute variation by hhld
asd <- df %>% select(new.income.pc,sq.income.pc,perno,N.fam, hhwgt.2016) %>% filter(perno == 1) %>% 
...
inc.quartiles <- with(df[df$perno==1,],
                      wtd.quantile(x = fam.sq.income, probs = c(.25, .5, .75), weights = hhwgt.2016*N.fam))


abline(v = inc.quartiles, col = "blue", lty =3)

losses <- (wage.gain.total["Total wage gain"])*1e9 

asd1 <- with(df, findInterval(x = sq.income.pc, vec = c(-Inf,11740, 6*11740, Inf)))

asd2  <- losses * c(0.30, 0.29, 0.41) / wtd.table(asd1, weights = df$hhwgt.2016)[[2]]
...
asd2 <- df %>%  with( wtd.table(findInterval(sq.income.pc, c(-Inf, 11770*1:6, Inf)), weights = hhwgt.2016/1e6) )
...
deciles <- with(df[df$perno==1,],wtd.quantile(x = sq.income.pc, probs = 1:9/10, weights = hhwgt.2016*N.fam))
