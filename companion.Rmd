...
param.base.growth <- 0.024 * 1
param.wage.gr <- 1
param.worker.gr <- 1 
param.N <- 1
param.fract.minwage <- 1
param.noncomp <- 1
param.F.adj <- 1
param.av.wage.var <- 1
param.wages <- 1
...
wage.gr <- ( ( gr.factor("wages per worker", 2014, 2016) )^(1/3) - 1 ) * param.wage.gr
workers.gr <- ( ( gr.factor("workers", 2014, 2016) )^(1/3) - 1 ) * param.worker.gr

#SAME
half.gap <- wage.gr - param.base.growth
wage.gr.bins <- seq(param.base.growth, wage.gr + half.gap, length.out = 10)
...
# SAME
df$wages.final <- with( df, ifelse(w3.adj1> st.minw.2016[state,] * param.states.raise,
                              w3.adj1,
...
df <- df %>% mutate("pop_of_int" = (empl == 1 &
                      (selfinc == 0 & selfemp == 0) & 
...
param.eta.lit <- 1
param.factor.extrap <- 1
eta.lit <- - 0.1 * param.eta.lit
factor.extrap <- 1/3 * param.factor.extrap
...
stats2["% Salary below new MW ($P(\\hat{w} \\leq MW^{1})$)","Teen"]
stats3["$F_{adj}$", "Teen"]
stats3["$\\overline{\\%\\Delta w}$","Teen"]
...
#starting at 2.4%, then adjust by minimum wages of 2016 and get a anualized 
#growth of 2.9% for the lowest decile. 
wage.gr <- ( ( gr.factor("wages per worker", 2013, 2016) )^(1/4) - 1 ) * param.wage.gr
workers.gr <- ( ( gr.factor("workers", 2013, 2016) )^(1/4) - 1 ) * param.worker.gr
...
df$wages.final <- with( df, ifelse(wage.adj1> st.minw.2016[state,] * param.states.raise,
                              wage.adj1,
                              st.minw.2016[state,] * param.states.raise) ) * param.wages
#FROM NOW ON 'wages.final' is the variable that contains the forecasted wage in 2016 under the status quo
```  
</div>
...
df$hhwgt.2016 <- df$hhwgt * (1 + workers.gr)^4
...
# Tag those workers 
df$below_min <-with(df, ifelse(wages.final <= 10.10 & pop_of_int == 1,
                               1, 0) )
...
df$new.wage <- with(df, ifelse(wages.final<10.10 & pop_of_int %in% 1, 
                               10.10, 
                               wages.final) )

# Apply ripple effects
df$new.wage <- with(df, ifelse(wages.final>10.10 & wages.final<11.5 & pop_of_int==1, 
                               wages.final + 0.5 * (11.5 - wages.final),
                               ifelse(wages.final>8.7 & wages.final<=10.10 & pop_of_int==1,
                                        10.10 + 0.5 * (wages.final - 7.25),
                                       new.wage
                                        ) ) )
# Creates the same but using dplyr
# df <- df %>% mutate( new.wage = ifelse(wages.final>10.10 & wages.final<11.5, 
#                                      10.10 + 0.5 * (10.10 - wages.final), 
#                                      new.wage) )  %>%
#                 mutate( new.wage = ifelse(wages.final>8.7 & wages.final<10.10, 
#                                      10.10 + 0.5 * (wages.final - 7.25), 
#                                      new.wage) )
...
N_benes_compliance_below_min <-  sum(df$hhwgt.2016[(df$wages.final != df$new.wage.nocomp) & df$below_min==1], na.rm = TRUE)/1e6
N_benes_compliance_above_min <-  sum(df$hhwgt.2016[(df$wages.final != df$new.wage.nocomp) & df$below_min==0], na.rm = TRUE)/1e6
...
df$cut.wage <- with(df, ifelse(cut.job %in% 1, wages.final/2, wages.final) )
# cut jobs relative to sq, wage raise
df$new.wage.final <- with(df, ifelse(cut.job %in% 1, wages.final/2, new.wage.nocomp) )
...
df <- df %>% mutate("year.wage" = wages.final * (hrslyr * wkslyr) ) %>% 
      group_by(hhseq) %>% mutate("hhld.wage" = sum(year.wage, na.rm = TRUE),
                                 "hhld.non.wage" = sum(non.wage.2016, na.rm = TRUE), 
                                 "hhld.income" = hhld.wage + hhld.non.wage, 
                                 "N.fam"= n(), 
