...
# Introduction  
- Note on how the original report was made under a tight deadline. It should not be as an article to refute/confirm. The reader should approach the report as a living document to be benefited from incremental improvements.    

PA cannot be incorrect in the same sense that a paper can be incorrect. 


The report describes two policy estimates: the effects of minimum wage on employment and the distributional effects of minimum wage on wages and familiy income. This companion presents the details behind both policy estimates. 
...
To describe the methodology behind each of those four components we first describe the data used, the wage variable choosen, and the procedure used to forecast the wage ditribution of 2016 using from 2013.  
...
### Loading the data  
```{r loading data, eval=TRUE,echo=TRUE, warning=FALSE, message=FALSE, cache=FALSE}
...
```{r Descript stats1, eval=TRUE,echo=TRUE, warning=FALSE, message=FALSE, cache=FALSE}
library(dplyr)
library(foreign)
library(weights)
library(survey)
...
```{r Universe, eval=TRUE,echo=TRUE, warning=FALSE, message=FALSE, results="hide", error=FALSE, collapse=TRUE, fig.show='hold', cache=FALSE}
library(treemap)
...
asd <- df %>% 
        mutate("teen" = ifelse(age<20, "teen", "adult"), 
               "selfemp_inc" = 1 * (selfemp == 1 | selfinc == 1)) %>%  
                  group_by(lfstat,selfemp_inc, teen)  %>% 
            summarise("total" = sum(orgwgt, na.rm = TRUE)) 

asd$selfemp_inc[asd$lfstat!="Employed"] = NA 
asd$teen[asd$lfstat!="Employed"] = NA 
...
asd$color <-heat.colors( nlevels(asd$wage_c) )[asd$wage_c]
...
treemap(asd,
 ...
 fontsize.labels = c(12:8),
 #aspRatio= c(4,3), 
 palette = c(rev(heat.colors( nlevels(asd$wage_c) )),"#D3D3D3"), 
title.legend="number of employees",
...
Now we have predicted salaries and population for 2016, from the perspective of 2013 ($\widehat{w^{16|13}}$). The two relevant populations to define now are the number of low wage workers ($\widehat{ N_{lowwage} }$) and the number of workers that, under the status quo, would earn less than the new minimum wage ($\widehat{ N_{w\leq MW^{1}} }$). 
...
#### Tipped workers (TO BE COMPLETED) 
...
We estimate the proportion of workers that earn less than the their state's minimum wage in 2013 as a proxy for non compliers under the new minimum wage in 2016. The orginal reports mention that it comes up to 12% of the low wage population. Given that I do not have a definition for what the low wage population is, I will estimate the number of workers below the minimum wage and reported as a percentage of the target population. 
...
#### Not covered that might benefit (TO BE COMPLETED) 
...
```{r Target population, eval=TRUE,echo=TRUE, warning=FALSE, message=FALSE, results="hide", error=FALSE, collapse=TRUE, fig.show='hold', cache=FALSE}
library(treemap)
...
$$ 
\begin{align}
\widehat{ N^{final}_{adult} } &= \hat{g(2016|2013)} \times  \hat{N_{employed}} (2013) P(\hat{w} \leq MW^{1} ) \times (1 - \hat{\alpha_{1}} - \hat{\alpha_{2}}) \times P(Age > 19 | \hat{w} \leq MW^{1})
\end{align}
$$


PUT A TABLE HERE WITH THE ESTIMATES FOR EACH COMPONENT.   
...
It is unclear the precise mechanism used by CBO to choose the estimates for the labor demand elasticity. Later I will assume that it came from a meta-analysis and calibrate the weights of some of the meta-analysis cited in the report to reflect their choice.  

For now I take their estimate at face value: -0.1 with a ''likely range'' a range from 0 to -0.2 (''likely range'' is used throughout the report to estimate the expert judgment that the elasticity will be in that range 2/3 of the time)[^1]. The reasons provided in the report for this figure can be summarize as:  
  - More weight was given to studies that exploit across state variation (as opposed to over time country level variation).    
  - The estimates take in to account publication bias towards highly negative estimates.  
...
$$ 
\begin{align} 
\eta_{lit} &= \eta_{lit}^{0} \times F_{pub.bias} \times F_{large.variation} = -0.1
\end{align}
$$ 
  
Additionally, CBO provides two additional caveats that could be added to the analysis:  
 - Ripple effects on employment were assume to null as the result of two opposing effects: (i) ''ripple-wages'' would increase unemployment but (ii) substitution of marginally more productive workers for layed off workers below the new minimum wage would decrease unemploytment. THIS IS A "PRESUMED ZERO".    
 - CBO acknowledges that effects could be larger (in abs value) during recessions but do not predict a recession for 2016. No estimaton is provided of how much larger those effects would be in case of a recession.  
...
$$
\begin{align}
\eta^{adults}_{w\leq MW} = \eta^{teens}_{w\leq MW} \times F_{extrapolation}
\end{align}
$$  
...
The fraction of the population with a forecasted income below the new minimum wage ($p^{g}_{w\leq MW}$) is ADD HERE for teenagers and ADD HERE for adults.  
...
## Other factors  
CBO argues that a rise in the minimum wage would have effects in aggregate consumption and this in turn would have effects on employment. The overall effects is estimated in an increase in employment between 30,000 and 50,000 jobs (``a few tens of thousands of jobs''). A narrative argument is provided for the mechanisms behind this effect, but no detail is provided as to how the effect is actually estimated. 
...
stats2 <-  df  %>% 
  ...
  group_by(adult)  %>% 
  summarise( "N" = sum(orgwgt)/1e6,
             "% Empl Below MW" = mean( 1*(w3.adj.min <=10.10), na.rm = TRUE) * 100 , 
             "% Mean Wage Inc" = mean( ifelse(w3.adj.min <=10.10,
                                            (10.10 - w3.adj.min)/w3.adj.min, NA) , 
                                     na.rm = TRUE) * 100 ) 

aux1 <- df %>%
  select(wage3, age, state, orgwgt, peernuot)  %>%
  filter(pop_of_int == 1 & wage3 < 11.5) %>% 
  mutate("adult" = ifelse(age>20, "Adult", "Teen") )  %>% 
  group_by(adult)  %>% 
      summarise(
...
stats2 <- cbind(stats2, aux1[,2])

t(stats2)

# As it should have been computed according to doc
delta.e1 <- sum( stats2$N * stats2$`% Empl Below MW`/100 * ( 1 - stats2$`% of non compliers with adj`/100 - 0) *
       stats2$`% Mean Wage Inc`/100 * 0.1/(stats2$`% Empl Below MW`/100) *
       ((stats2$`% Mean Wage Inc`/100)/0.37) * c(1/3, 1) 
) - 0.05 * 0.9
# As described in doc
delta.e2 <- sum( stats2$N * stats2$`% Empl Below MW`/100 * ( 1 - stats2$`% of non compliers with adj`/100 - 0) *
       stats2$`% Mean Wage Inc`/100 * 0.1 * 4.5 * c(1/3, 1) ) - 0.05 * 0.9 

# "Arbitrary" Sens. Anal to get lower unemp (0.9 * wage in c, 0.37 to 0.5, 1/3 to 1/4)
delta.e3 <- sum( stats2$N * stats2$`% Empl Below MW`/100 * ( 1 - stats2$`% of non compliers with adj`/100 - 0) *
       0.9*stats2$`% Mean Wage Inc`/100 * 0.1/(stats2$`% Empl Below MW`/100) *
       ((0.9*stats2$`% Mean Wage Inc`/100)/0.5) * c(1/4, 1) 
) - 0.05 * 0.9
...
Using all the components descbribed above we get $\widehat{ \Delta^{-} E } =$ `r round(delta.e1*1000)` thousand jobs. The report however computes $F_{adjs}$ in a different fashion and gets a value of 4.5[^2]. Plugging this value into the equation above one gets $\widehat{ \Delta^{-} E } =$ `r round(delta.e2*1000)` thousand jobs. [FH: let's not get super excited yet. I suspect that several components of my estimation are different from the ones in the CBO report (e.g. non-compliance rate)]
...
The hourly wage ($w$) was computed as the ratio of yearly earnings ($y$) and the product of usual number of hours worked in a week ($Hour.per.Week$) and the number of weeks worked in a year ($Weeks.per.Year$). The CPS ASEC data set contains three variables for yearly earnings: `incp_all` `incp_ern` `incp_wag` corresponing to all income, earnings and wages respectively. We choose `incp_ern`. 

$$
\begin{align}
w = \frac{\hat{y}}{\hat{Hour \, per \, Week} \times \hat{Weeks\,per\,Year}}
\end{align}
$$
...
# Tag population of interest FH: Need to deal with 598 NA's
pop_of_int <- with(df,
                   (empl == 1 &
                      (selfinc == 0 & selfemp == 0) & 
                      !(rincp_wag == 0 | is.na(rincp_wag) ) )
  )
...
table_6 <- df %>% 
  filter(pop_of_int == 1 & !is.na(hhwgt)) %>%
    select(rincp_wag, hrslyr, wkslyr, hhwgt) 

table_6 <- sapply(table_6[,c("rincp_wag", "hrslyr", "wkslyr")],function(x) sum.stas1(x, table_6$hhwgt) )
table_6 <- cbind(table_6)
colnames(table_6) <- c("Earnings", "Hours", "weeks") 

df$wage <- with(df, rincp_wag/(hrslyr * wkslyr) )
df$wage[df$wage<0] = NA
table_7 <- df %>% 
  filter(pop_of_int == 1 & !is.na(wage)) %>% 
    summarise("> $7.5" = weighted.mean(wage<7.5,w = hhwgt), 
              "> $9" = weighted.mean(wage<9,w = hhwgt), 
              "> $10.10" = weighted.mean(wage<10.10,w = hhwgt), 
              "> $13" = weighted.mean(wage<13,w = hhwgt), 
              "> $15" = weighted.mean(wage<15,w = hhwgt) 
              )

table_7 <- t(table_7)
colnames(table_7) <- "Perc"
...
#### Adjusting wages 2  
CBO mentions that the "it found far fewer workers who would be directly affected by the change in the minimum wage than it had in its analysis of employment"
This is not the case in our estimations here: using the CPS ASEC we get `r round(table_7[ "> $10.10",]*100, 1)` workers below the 10.10 threshold, while using the CPS ORG we `r round(table_3[ "> $10.10",]*100, 1)`. 
...
Need to define which group of workers I will increase the wages, and fraction of workers to be affected ($a,b$),  
...
$$
\begin{align}
\widehat{Y_{h}(2016|2013)} &= \sum_{i \in h} ( g_{w}\hat{w} + \sum_{l}(g_{nw_{l}}\hat{nw_{l}}) ) 
\end{align}
$$ 

The wage forecast is the same process as in section 2, the key difference is that is applied to a different data set (CPS ASEC). The other components of family income were forecasted as follows: when a growth rate was available for the subcomponent it was applied (the only one mentioned is interest and dividends), otherwise the growth rate was equal to the change in the price index for personal consumption (TO BE COMPLETED)
...
#### Code to get economic growth forecasts  
```{r Getting forecast data ADD INCOME GROWTH, eval=TRUE,echo=TRUE, warning=FALSE, message=FALSE, results = "hide"}
# All projections data comes from this website: https://www.cbo.gov/about/products/budget_economic_data
# name of the files that contain projections from CBO
early.2016  <- "51135-2016-01-Economic%20Projections.xlsx"
late.2015   <- "51135-2015-08-EconomicProjections.xlsx"
...
#### Code to get minimum wage values by state  
```{r Getting min wage data DUPLICATED, eval=TRUE,echo=TRUE, warning=FALSE, message=FALSE, results = "hide"}
# Minimum wage by state: 
# Check if data is in machine and download if not.
if ( !("minwage" %in% dir()) ) {
  library(XML)
...
#### Code to forecast wages, workers and non-wage income
```{r Adjusting income to 2016 , eval=TRUE,echo=TRUE, warning=FALSE, message=FALSE, results = "hide"}
library(dplyr)
library(foreign)
library(weights)
library(survey)
...
#IMPORTANT NOTE: I SUSPECT THAT THE FORECAST SHOULD BE FROM 2012 in ASEC, VERIFY LATER
#CBO mentions that the lowest 10th percent gets a 2.9% growth in anual wage
#I compute the anualized growth rate of wages and creat 10 bins of wage growth
#starting at 2.4%, then adjust by minimum wages of 2016 and get a anualized 
#growth of 2.9% for the lowest decile. 
...
non.wage.gr <- ( gr.factor("Price Index, Personal Consumption", 2014, 2016) )^(1/3) - 1
...
df <- df %>%
        mutate( wage.deciles = cut(wage, c(0, aux.var, Inf) , 
                                right = TRUE, include.lowest = TRUE) ,  
                wage.adj1 =  wage * ( 1 + wage.gr.bins[wage.deciles] )^3) 
...
#1.1Separate HH income in to wage and non-wage
df$non.wage <- (df$rincp_ern - df$rincp_wag) 
df$non.wage <- df$non.wage * gr.factor("Price Index, Personal Consumption", 2014, 2016)


```  


```{r desc stats2 in income,  eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
# WHY TABLE 4?
...
table_8[3:7,1]  <- round(as.matrix(table_7), digits = 2)

aux.1 <- df %>% 
  filter(pop_of_int == 1 & !is.na(wage.adj.min)) %>% 
    summarise("> $7.50" = weighted.mean(wage.adj.min<7.5,w = hhwgt * (1 + workers.gr)^3), 
              "> $9" = weighted.mean(wage.adj.min<9,w = hhwgt * (1 + workers.gr)^3), 
              "> $10.10" = weighted.mean(wage.adj.min<10.10,w = hhwgt * (1 + workers.gr)^3), 
              "> $13" = weighted.mean(wage.adj.min<13,w = hhwgt * (1 + workers.gr)^3), 
              "> $15" = weighted.mean(wage.adj.min<15,w = hhwgt * (1 + workers.gr)^3) 
              )
...
# Histogram of wage below $20
df  %>% 
  filter(pop_of_int==1 & wage<=20)  %>% 
    with(wtd.hist(wage, breaks = 50, 
                  xlab = "Hourly Wage", 
                  freq = TRUE, 
                  col=rgb(1,0,0,0.4),
                  main = "Distribution of Hourly Wages Below $20",
...
df  %>% 
  filter(pop_of_int==1 & wage.adj.min<=20)  %>% 
    with(wtd.hist(wage.adj.min, breaks = 50, 
                  xlab = "Hourly Wage", 
                  freq = TRUE, 
                  main = "Distribution of Hourly Wages Below $20", 
                  col=rgb(0,0,1,0.4), 
                  weight =  hhwgt * gr.factor("workers", 2014, 2016),
                  add = TRUE) ) 

abline(v = c(7.25, 9, 10.10), col = "red")
...
# Compute total wage increase (yearly in billions)
wage.inc <- df %>% 
    filter(below_min == 1)  %>% 
      with( sum((10.10 - wage.adj.min) *  hhwgt * uhours * weeks , na.rm = TRUE) ) / 1e9 

# Create new wage (after inc in min wage)
df$new.wage <- with(df, ifelse(wage.adj.min<10.10, 
                               10.10, 
                               wage.adj.min) )
...
#                                      new.wage) )
df$new.wage <- with(df, ifelse(wage.adj.min>10.10 & wage.adj.min<11.5, 
                               10.10 + 0.5 * (10.10 - wage.adj.min)
                               ,ifelse(wage.adj.min>8.7 & wage.adj.min<10.10,
                                        10.10 + 0.5 * (wage.adj.min - 7.25),
                                       new.wage
                                        ) ) )
...
num.to.del <- delta.e2*1e6*2

theta <- 0.4
cut.job <- 0
while ( abs(sum(cut.job * df$hhwgt , na.rm = TRUE) - num.to.del) >= 1000)  { 
  set.seed(123)
  cut.job <- ( ( exp(m_x(.1, .01)) / (1 + exp( m_x(.1 ,.01) )  ) >= 
                   theta + .2 * runif(dim(df)[1], min = 0, max = 1) ) )
...
df$new.wage <- with(df, ifelse(cut.job == 1, new.wage/2, new.wage))
...
wage.loss.total <- with(df, sum( (wage.adj.min - cut.wage) * 
...
# Family income under status quo

# Family income under new min wage (incl. wage gains and employment loses)
df$sq.wage <- with(df,wage.adj.min * (hrslyr * wkslyr))
df <- df %>% group_by(hhid) %>% mutate(fam.sq.wage = sum(sq.wage, na.rm = TRUE))

df$sq.income <- with(df, fam.sq.wage + non.wage)

df$new.wage.total <- with(df,new.wage * (hrslyr * wkslyr))
df <- df %>% group_by(hhid) %>% mutate(fam.new.wage = sum(new.wage.total, na.rm = TRUE))

df$new.income <- with(df, fam.new.wage + non.wage)

# get per capita
df <- df %>% group_by(hhid)  %>% mutate(N.fam= n())
# select one per family 
df %>% select(new.income,sq.income,perno,N.fam, hhwgt) %>% 
  with(sum( (new.income - sq.income) * hhwgt/N.fam , na.rm = TRUE) ) /1e9
# Compute variation by hhld
asd <- df %>% select(new.income,sq.income,perno,N.fam, hhwgt) %>% filter(perno == 1) %>% 
  mutate( "variation" = (new.income - sq.income)/N.fam) 

x11()
with(asd,plot(sq.income/N.fam, variation, xlim = c(0,2e5), 
              col = rgb(0,0,0,0.1) , pch =19, cex = .2) )
abline(v = 11740*1:6, col = "red", lty = 2)



qwe <- with(df[df$perno==1,],wtd.quantile(x = sq.income/N.fam, probs = 1:10/10, weights = hhwgt))
abline(v = qwe, col = "blue", lty =3)

losses <- (wage.gain.total - wage.loss.total)*1e9

asd1 <- with(df, findInterval(x = sq.income/N.fam, vec = c(-Inf,11740, 6*11740, Inf)))

asd2  <- losses * c(0.30, 0.29, 0.41) / wtd.table(asd1, weights = df$hhwgt)[[2]][-1]

asd3 <- asd2[with(asd, findInterval(x = sq.income/N.fam, vec = c(-Inf,11740, 6*11740, Inf) )) ]

with(asd, points(sq.income/N.fam ,-asd3,  xlim = c(0,2e5), 
              col = rgb(0,0,1,0.1) , pch =19, cex = .2) )


#Average income per group (height of rectangle)
asd1 <- df %>% group_by(findInterval(sq.income, c(-Inf, 11770*1:6, Inf) )) %>% summarise(log(mean(new.income)))

#number of individuals per group (width of recangle)
asd2 <- df %>%  with( wtd.table(findInterval(sq.income, c(-Inf, 11770*1:6, Inf)), weights = hhwgt/1e6) )


#Add gains in wages 
#Add losses in wages
#Add losses in profit and inc prices. 
...
table_9 <- matrix(NA, ncol = 7, nrow = 4)
colnames(table_9)  <- paste("I_", 1:7, sep = "")
...
income.group <- with(df, findInterval(x = sq.income/N.fam, vec = c(-Inf,1:6*11740, Inf)))
table_9["N_i", ] <- wtd.table(with(df, income.group) , weights = df$hhwgt)[[2]]/1e6
table_9[ "wage loses", ] <- 

df %>% group_by(income.group)  %>% summarise(mean(new.wage - wage.adj.min, na.rm = TRUE))  

#FH: questions for Phi:
