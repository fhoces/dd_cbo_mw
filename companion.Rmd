...
1 - The role of policy analysis is to connect the research with policy.
2 - There is a lot of bullshit in policy analysis. 
3 - This creates three problems:
    3.1 - Makes it hard to understand the connection between research and policy. 
    3.2 - Allows policy makers to cherry pick policy reports. 
    3.3 - Difficults systematic improvents or automation of analysis. 
4 - This document is a demonstration of how to take the bullshit out of one particular policy analysis, and how doing so addresses the problems described above. 
...
Here we attempt to contribute to the policy discussion of the minimum wage. Every policy discussions has a empirical and a normative component. Policy makers should debate on the normative field. If there is much ambiguity in the empirical component, it is much harder to have a separate conversation about the normative differences across parties. When some details (or in some cases the entire process) behind the computation of policy estimates are left undescribed, policy makers can confound differences in values with differences in interpretation of the evidence. 
  
  

Add clarity to the technical discussion of minimum wage.  

There are so many pieces 

I just want to reduce the space for bullshit



The minimum wage is an contentious policy issue in the US. Increasing it has possitive and negative effects that different policy makers value differently. When there is claritiy on what those effects are the conversation can be focused on how the valuations vary across policy makers, and what    But what are the costs and benefits of increasing the minimum wage and who bares them? 

In 2014 the Congressional Budget Office published the most up-to-date, non-partissan, policy analysis of the effects of raising the minimum wage on employment and family income. The report receive wide attention from the media and has been used extensible as an input in the debate around the minimum wage. Here we use that report as the starting point of Although there was some disagreement among experts around some technical issues, this disagrement has been mainly circunscribed around of the many inputs used in the analysis. 

Opinions on the minimum wage, like almost any policy discussion, are a product of combining evidence and subjective valuations of such evidence. 

This (living) document is a reader companion to the report form the Congressional Budget Office titled ["The Effects of a Minimum-Wage Increase on Employment and Family Income"](https://www.cbo.gov/publication/44995). Our purposes are twofold: (i) promote the technical discussion around a recurrent policy issue (minimum wage) by making explicit and visible all the components (e.g. data, codes, methods) and key assumptions of the most up-to-date official policy analysis on the matter and (ii) demonstrate how new scientific practices of transparency and reproducibilty (T & R) can be applied to policy analysis. We encourage readers to collaborate in this document and help develop an ever-improving version of the important policy estimates[^3] (re)produced here.

To achieve our goal we reviewed the CBO report and extract the key components of its analysis. For our analysis we follow the guidelines propose by (Me? CITE dissertation) where the analysis achieves the highest standards of T & R when the data, methods and workflow are completly reproducible and every part of the analysis, and its assumptions, are easily readable. 

CBO's report, in its original form already represents significant inprovement in T & R relative to the standard practices of policy analyses. The report contain almost all the components required for a full replication. We add some missing components, make explicit assumptions when needed, complement the narrative explanations with some mathematical formulae, a few visualizations, and the analytical code use behind all the replication. 


Althoughour aim is to translate practices of T & R from Science to Policy Analysis, we need to highlight a important difference between a replication in science and a replication in policy analysis. A scientific report takes the form of a peer review publication that represent several months or years of research, followed up by a review process that can be as lengthy as the research itself. Because of this when a scientific publication is subject to replication is expected to succeed. Policy analysis is usually performed under tight deadlines, and is not unusual to rely on arbitrary assumptions and/or unreproducible calculations. For this reasons we do not attempt to replicate the CBO report as a way of testing the veracity of the analysis. We use reproducibility, paired with full transparency, to generate a living document that represents the best policy analysis up to date. Our expectations are that this living document will be serve as a building block to discuss and incorporate incremental improvements on the best available policy analysis. 
...
#### Code to load the data (`Stata`)  

<a id="displayText" href="javascript:toggle(1);">Code</a>  
...
##### Code to get economic growth forecasts (`R`)    
...
##### Code to get economic growth forecasts (`Stata`)   

<a id="displayText" href="javascript:toggle(2);">Code</a>  
...
##### Code to get minimum wage values by state (`R`)  
...
##### Code to get minimum wage values by state (`Stata`)  
<a id="displayText" href="javascript:toggle(3);">Code</a>  
...
#### Code to forecast wages and workers (`R`)
...
#### Code to forecast wages and workers (`Stata`)
<a id="displayText" href="javascript:toggle(4);">Code</a>  
...
#### Statistics and code behind figure 1 (`R`)

```{r desc stats2,  eval=TRUE,echo=display_code, warning=FALSE, message=FALSE, results="hide", error=FALSE, collapse=TRUE}
...
#### Statistics and code behind figure 1 (`Stata`)

<a id="displayText" href="javascript:toggle(5);"><h4>Statistics and code behind figure 1 (`Stata`)</h4></a>  
...
For the universe of interest (employed, salaried, with hourly wages or non varying hours N = `r temp.pop.of.interest` millions), we describe the distribution of hourly wages in 2013 and the forecast values for 2016. Figure 2.  AQUI VOY  
...
```{r, eval=TRUE,echo=display_code, warning=FALSE, message=FALSE, error=FALSE, collapse=TRUE, fig.show='hold'}
...
universe.1$wage_n <- as.factor(universe.1$wage_c)

universe.1$selfemp_inc[universe.1$selfemp_inc==0]  <- "salary"
universe.1$selfemp_inc[universe.1$selfemp_inc==1]  <- "self employed or self incorporated"

universe.1$color <-c("#fff7bc","#d95f0e")[as.factor(universe.1$wage_c)] 
...
#### Tipped workers   
[TO BE COMPLETED] Ask CBO which occupations they used to identify tipped workers and clarify the conceptual need to adjust for this population.    
...
We estimate the *proportion of low workers that earn less than the their state's minimum wage in 2013* as a proxy for non compliers under the new minimum wage in 2016. The original reports mention that it comes up to 12% of the low wage population. 
...
```{r Target population, eval=TRUE,echo=display_code, warning=FALSE, message=FALSE,  error=FALSE, collapse=TRUE, fig.show='hold'}
library(treemap)
...
In the first step towards obtaining the policy estimates presented in the [introduction](#introduction) we concluded with a figure of $\widehat{ \Delta^{-} E } =$ `r round(delta.e1*1000)` thousand jobs lost.  We now two additional key quantities: the wage gain among those who get a rise do to the new minimum wage, and the distribution of the losses that pay for that raise. The effect of both quantities is estimated at the level of family income.  
...
# The following function takes hourly, individual wages (already forecasted to 2016) and does the folowing:
# 1 - transform hourly wages in to yearly earnings
# 2 - aggregate earnings at the hhld level
# 3 - adds the hhld level wages to non wages earnings (hhld level) to generate overall income
# 4 - generates income per capita
...
asd <- df %>% select(new.inc.pc,sq.inc.pc, hhwgt.2016)  %>% 
  mutate( "variation" = new.inc.pc - sq.inc.pc ) %>% select(variation, sq.inc.pc)

#x11()
with(asd,plot(sq.inc.pc, variation, xlim = c(0,2e5), ylim = c(-5e3, 5e3), 
              col = rgb(0,0,0,0.1) , pch =19, cex = .2, 
              main = "Distribution of gains and loses across per capita income", 
              xlab = "Per capita income", 
              ylab = "Change relative to no raise in min wage") )
abline(v = 11740*1:6, col = "red", lty = 1, cex=1.3)

inc.quartiles <- with(df,
                      wtd.quantile(x = sq.inc.pc, probs = c(.25, .5, .75, .90), weights = hhwgt.2016))


#abline(v = inc.quartiles, col = "blue", lty =1, cex=3)

#Substract 2billion dollars from the losses 
losses <- (wage.gain.total["Total wage gain"])*1e9  - 2e9

asd1 <- with(df, findInterval(x = sq.inc.pc, vec = c(-Inf,11740, 6*11740, Inf)))

asd2  <- as.numeric(losses) * c(0.01, 0.29, 0.70) / wtd.table(asd1, weights = df$hhwgt.2016)[[2]]

asd3 <- asd2[with(asd, findInterval(x = sq.inc.pc, vec = c(-Inf,11740, 6*11740, Inf) )) ] 

with(asd, points(sq.inc.pc ,-asd3,  xlim = c(0,2e5), 
...
deciles <- with(df,wtd.quantile(x = sq.inc.pc, probs = 1:9/10, weights = hhwgt.2016))

df$income.group <- with(df, findInterval(x = sq.inc.pc, vec = c(-Inf,c(1,3,6)*11740, Inf)))

table_9["N_i", ] <- wtd.table(with(df, income.group) , weights = df$hhwgt.2016)[[2]]/1e6
  
aux.1 <- df %>% group_by(income.group)  %>% summarise(sum((new.inc.pc - sq.inc.pc) * hhwgt.2016, na.rm = TRUE)/1e9)
...
```{r final results, eval=TRUE,echo=display_code, warning=FALSE, message=FALSE}

output.template1.f <- cbind(output.template1, "Replication" = c(
  round(as.numeric(wage.gain.total["Total wage gain"]), 1),  
  round(as.numeric(wage.gain.total["Total wage loss"]), 1), 
  round(as.numeric(wage.gain.total["Total wage gain"]), 1) - 2,  
  2,  
...
output.template2.f <- cbind(t(output.template2), "Replication loses" =  -round((table_9[ "other loses", ]), 1) 
                          ,"Replication NE" = round(table_9[ "wage gains", ], 1) )
