...
universe.1$teen[universe.1$lfstat!="Employed" | universe.1$selfemp_inc==1] = NA 
...
if (FALSE) {
    ...
    filter(pop_of_int==1 & wage3<=200)  %>% 
      select(wage3, w3.adj.min) %>% 
      melt() %>%  
  ggplot(aes(value)) + 
    coord_cartesian(xlim = c(0,30), ylim = c(0 , 1e7)) +
    geom_density(aes(weights=final_weights,color=variable, fill=variable, ), bw = 1) +
ggplot(aes(w3.adj.min)) + 
  ...
  geom_density(aes(weights=final_weights),
      ...
      with( plot(density(w3.adj.min, 
                  weight =  final_weights
                  ),                  col=rgb(1,0,0,0.4),
                  main = "Distribution of Hourly Wages Below $20",                  xlab = "Hourly Wage", 
