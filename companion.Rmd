...
```{r table with output,  eval=TRUE,echo=FALSE, warning=FALSE, message=FALSE, cache=FALSE}  
#This is summary of all the policy estimates presented in the report
#Aggregate effects
output.template1 <- matrix(" ???",nrow = 7, ncol = 1)
rownames(output.template1) <- c("wage gains", "wage losses", "Balance losses", "Net effect", "# of Wage gainers", "#of Wage losers", "Population")
...
## Get the $N$    
According to the CPS data the population of working age in 2013 was `r round( sum(df$final_weights, na.rm = TRUE)/1e6, 1)` million*. Of those, `r round( sum( df$final_weights * (df$lfstat == "Employed"), na.rm = TRUE)/1e6, 1)` million were working, `r round( sum( df$final_weights * (df$lfstat == "Unemployed"), na.rm = TRUE)/1e6, 1)` and `r round( sum( df$final_weights * (df$lfstat == "NILF"), na.rm = TRUE)/1e6, 1)` where not in the labor force (NILF). 
...
Now we have predicted salaries and population for 2016, from the perspective of 2013 ($\widehat{w^{16|13}}$). The two relevant populations to define now are the number of low wage workers ($\widehat{ N_{lowwage} }$) and the number of workers that would earn less than the new minimum wage ($\widehat{ N_{w\leq MW^{1}} }$). 
...
stats2 <-  df  %>% 
  ...
  mutate("adult" = ifelse(age>=20, "Adult", "Teen") )  %>% 
  group_by(adult)  %>% 
  summarise( "N" = sum(final_weights)/1e6,
...
# Family income under status quo and with wage loss
...
df <- df %>% group_by(hhid) %>% mutate(fam.cut.wage = sum(cut.wage.total, na.rm = TRUE))

df$sq.income <- with(df, fam.sq.wage + non.wage)
df$income.cut.wage <- with(df, fam.cut.wage + non.wage)
...
inc.quartiles <- with(df[df$perno==1,],wtd.quantile(x = sq.income/N.fam, probs = c(.25, .5, .75), weights = hhwgt*N.fam))
...
deciles <- with(df[df$perno==1,],wtd.quantile(x = sq.income/N.fam, probs = 1:9/10, weights = hhwgt*N.fam))

df$income.group <- with(df, findInterval(x = sq.income/N.fam, vec = c(-Inf,c(1,3,6)*11740, Inf)))

table_9["N_i", ] <- wtd.table(with(df, income.group) , weights = df$hhwgt)[[2]]/1e6
  
aux.1 <- df %>% group_by(income.group)  %>% summarise(mean((new.income - sq.income)/N.fam, na.rm = TRUE))
table_9[ "wage gains", ] <- t(as.data.frame(aux.1[,2]) )
  
  
#Wage loses: compared to SQ
aux.1 <- df %>% group_by(income.group)  %>% summarise(mean((sq.income - income.cut.wage)/N.fam, na.rm = TRUE))  
